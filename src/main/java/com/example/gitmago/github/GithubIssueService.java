...
public class GithubIssueService {
    ...
    public int getIssueCount(User user) {
        String token = user.getGithubAccessToken();
        String username = user.getGithubUsername();
        String fromDate = user.getExpireAt().format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"));

        String graphqlQuery = """
            {
              search(query: "type:issue author:%s created:>=%s", type: ISSUE, first: 100) {
                issueCount
              }
            }
